(function (Factory) {
    ...
    class Datepicker {
        constructor([selector]) {
            ...
            this.sDate = this.selector.querySelector('[data-name="selectedDate"]'); this.render();
        }
        ...
        render(d = this.Dates.date(), m = this.Dates.mIndex(), y = this.Dates.year()) {
            ...
            for (let d = 0; d < mLength + (firstDay - 1); d++) {
                let cell = document.createElement("td"); if (d + 1 > firstDay - 1) $(cell).append(`<div class="date-item">${d + 1 - firstDay + 1}</div>`)
